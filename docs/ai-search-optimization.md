...
Missing: 1     Buried: 14    MRR: 0.603
P@5: 0.252     Deck%: ~74%   耗时: ~156ms
```

**阈值评测（120 条用例，candidateLimit=500）**：

| 阈值 | Precision | Recall(S) | Recall(A) | Empty% | avg | p50 |
|------|-----------|-----------|-----------|--------|-----|-----|
...
**结论**：`minScore=0.30` 是最佳平衡点 — Recall 90%、Empty 仅 3.3%，结果数量自然随查询质量变化。
