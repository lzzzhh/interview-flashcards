// Neo4j V4 Enhanced Recall — 2-hop graph traversal, bilingual token matching,
// concept-diverse result dedup. Based on v1 structure + V4 keyword tiers.
...
export async function neo4jHybridSearchV4(input: HybridSearchInput): Promise<CardMatch[]> {
  ...
  // 2. Neo4j graph-enhanced query expansion (V4: 2-hop + bilingual)
  const { tiers: neo4jTiers, context: neo4jContext } = await neo4jBuildKeywordTiersV4(input.query);
  ...
  const graphContext: Neo4jGraphContext = {
    ...
    keywordTiers: {
      ...
      lowPriorityKeywords: neo4jTiers.lowPriorityKeywords,
    },
    graphScore: 0,
    graphScoreBreakdown: {},
  };
  ...
  // 8. Rerank
  const profile = input.overrideProfile || detectProfile(input.query, topic, mergedExpandedKeywords.slice(0, 5), input.deckIds || []);
  ...
  // 8d. Concept-diverse reordering: spread same-concept cards
  // (soft dedup — not gating, just penalize clustering in Top15)
  const conceptSeen = new Map<string, number>();
  const MAX_CONCEPT_IN_TOP = 4;
  for (let i = 0; i < ranked.length; i++) {
    const card = cardMap.get(ranked[i].cardId);
    if (!card) continue;
    const concept = (card.canonicalConcept || card.canonicalTopic || card.title || '').toLowerCase().split(',')[0].trim();
    if (!concept) continue;
    const seen = conceptSeen.get(concept) || 0;
    if (seen >= MAX_CONCEPT_IN_TOP) {
      ranked[i].finalScore *= 0.95; // slight penalty for 5th+ occurrence
    }
    conceptSeen.set(concept, seen + 1);
  }
