...
const INTENT_PATTERNS: { intent: SearchIntent; patterns: RegExp[] }[] = [
  { intent: 'study',   patterns: [
    ...
    // Compound: X不会Y → extract topic AFTER 不会
    /^.+?(?:不会|不懂).{0,2}(.+)(?:怎么补|怎么学|，补哪些|补什么)/,
    // Interview-based: X总答不好 → extract topic AFTER 总答不好
    /^.*?(?:总[答做]不好|被问到|面试).{0,2}(.+)(?:怎么补|.*补哪些卡|补什么)/,
    // Weakness: X很薄弱 → extract topic BEFORE 很薄弱
    /^(.+?)(?:很薄弱|不太懂|老是搞混|完全没概念).*/,
  ]},
