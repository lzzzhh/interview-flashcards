// Neo4j-enhanced hybrid search — uses Neo4j knowledge graph for concept
// lookup, query expansion, and graph scoring, then combines with FTS5/tag
// recall and the shared reranker.
//
// Kept fully isolated from hybrid-search.ts to enable side-by-side evaluation.
...
export async function neo4jHybridSearch(input: HybridSearchInput): Promise<CardMatch[]> {
