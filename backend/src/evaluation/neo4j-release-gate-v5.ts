...
async function main() {
  ...
  const neo4jResult = await runEval('neo4j-v1', async (query) => {
    return neo4jHybridSearch({
      query, maxResults: BASELINE_SEARCH_CONFIG.maxResults,
      minScore: BASELINE_SEARCH_CONFIG.minScore,
      candidateLimit: BASELINE_SEARCH_CONFIG.candidateLimit,
    });
  });

  console.log('[neo4j-v5] Running LLM rerank search...');
  const neo4jV5Result = await runEval('neo4j-v5', async (query) => {
    return neo4jHybridSearchV5({
      query, maxResults: BASELINE_SEARCH_CONFIG.maxResults,
      minScore: BASELINE_SEARCH_CONFIG.minScore,
      candidateLimit: BASELINE_SEARCH_CONFIG.candidateLimit,
    });
  ...
  console.log(`${'Metric'.padEnd(10)} ${'Baseline'.padEnd(12)} ${'Neo4j'.padEnd(12)} ${'Delta'.padEnd(12)} Status`);
  console.log('-'.repeat(70));

  for (const m of metrics) {
    const delta = m.higher ? m.neo - m.base : m.base - m.neo;
    const better = m.higher ? delta > 0 : delta < 0;
    const status = delta === 0 ? '=' : better ? '+BETTER' : '-WORSE';
    console.log(
      `${m.name.padEnd(10)} ${m.fmt(m.base).padEnd(12)} ${m.fmt(m.neo).padEnd(12)} ${(delta > 0 ? '+' : '')}${m.higher ? m.fmt(delta) : m.fmt(-delta)} ${' '.repeat(6)}${status}`
    );
  }
  ...
  console.log(`Neo4j:    Top15=${(neo4jResult.top15*100).toFixed(1)}% MRR=${neo4jResult.mrr.toFixed(3)} Missing=${neo4jResult.missing}`);
  console.log('='.repeat(70));
